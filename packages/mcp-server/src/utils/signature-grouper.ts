 ...
 *
 * Error-pattern matching, signature composition, and location formatting
 * were inlined from the former `error-normalizer.ts` in v0.9.2 US-009 so
 * the file's only public consumer (`tools/auto-optimize.ts` via
 * `groupBySignature` / `formatGroups` / `calculateStats`) owns its entire
 * dependency chain. Byte-parity against the 1,000-line fixture snapshot is
 * locked by `signature-grouper-parity.test.ts`.
 */
 ...
 */
function extractErrorParts(line: string): ErrorParts | null {
 ...
 */
function createSignature(parts: ErrorParts): string {
 ...
 */
function formatLocation(parts: ErrorParts): string {
