...
**Symptoms:** Claude Code doesn't show Distill tools
...
3. Verify the server name matches in `~/.claude.json`
...
2. Try a specific strategy: `strategy="build"` or `strategy="logs"`
3. Use `aggressive: true` for maximum compression
